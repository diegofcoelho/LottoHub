...
angular.module("materialCalendar").directive("calendarMd", ["$compile", "$parse", "$templateRequest", "$q", "materialCalendar.Calendar", "MaterialCalendarData", function ($compile, $parse, $templateRequest, $q, Calendar, CalendarData) {
    ...
    return {
        ...
        link: function ($scope, $element, $attrs) {
            ...
            // Add the CSS here.
            injectCss();
            // define minDate default here, separated from the others, to use it to determine current month and year
            // based on it
            ...
            if ($scope.minDate) {
                month = parseInt($scope.minDate.month());
                year = parseInt($scope.minDate.year());
            } else {
                month = parseInt($attrs.startMonth || date.getMonth());
                year = parseInt($attrs.startYear || date.getFullYear());
            }
            ...
            $scope.isDisabled = function (date, startDateOfMonth, noOfDays) {
                ...
                date.setHours(23);
                date.setMinutes(59);
                if (($scope.minDate !== null) && moment(date).isSameOrBefore($scope.minDate)) {
                    return true
                }
                date.setHours($scope.minDate.hour());
                date.setMinutes($scope.minDate.minute() + 5);
                if (($scope.disablePastSelection && date < new Date()) && (moment(date).format("DD/MM/YYYY") !== moment(new Date()).format("DD/MM/YYYY"))) {
                    return true;
                }
